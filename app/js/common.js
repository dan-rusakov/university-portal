$(function() {
	...
	// open/close mobile search line 

	var inputBoxMobile = $('.search-line-mobile__input');
	var searchBoxMobile = $('.search-line-mobile');

	$('.search-link').on('click', function(){
		searchBoxMobile.addClass('active');
	});

	$('.search-line-mobile__close').click(function(){
		var inputVal = inputBoxMobile.val();
		inputVal = $.trim(inputVal).length;
		if( inputVal !== 0){
			inputBoxMobile.val("");
		} else {
	...
	$('.search-line-mobile__btn').click(function(){
		var inputVal = inputBoxMobile.val();
		inputVal = $.trim(inputVal).length;
		if( inputVal !== 0){
			$(this).click();
	...
	$('.open-login-popup').fancybox({
		
	});

	// change login/register popup

	$('.login-popup__tab-box').click(function(){
		var tabBoxIdx = $(this).index();
		$('.login-popup__tab-box').removeClass('active');
		$(this).addClass('active');

		if(tabBoxIdx == 1){
			$('.login-popup__login-form').css('display', 'grid');
			$('.login-popup__register-form').css('display', 'none');
		} else {
			$('.login-popup__login-form').css('display', 'none');
			$('.login-popup__register-form').css('display', 'grid');
		}
	});
